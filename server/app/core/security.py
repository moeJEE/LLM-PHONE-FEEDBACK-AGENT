...
async def get_clerk_jwks():
    """Fetch and cache Clerk's JSON Web Keys"""
    now = time.time()
    
    if clerk_jwk_cache["keys"] and clerk_jwk_cache["expires_at"] > now:
        return clerk_jwk_cache["keys"]
    
    # Clerk instance should be configured in .env
    clerk_instance = settings.CLERK_INSTANCE_ID or "your-clerk-instance"
    url = f"https://{clerk_instance}.clerk.accounts.dev/.well-known/jwks.json"
    ...
    async with httpx.AsyncClient() as client:
        ...
        clerk_jwk_cache["expires_at"] = now + 3600  # Cache for 1 hour
